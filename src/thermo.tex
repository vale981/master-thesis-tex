...
\label{sec:therm_results} After the precision studies of
\cref{sec:hopsvsanalyt,sec:prec_sim} we turn to applications of the
formalism developed in \cref{chap:flow} that are related to the
thermodynamic task of extracting energy from systems coupled to heat
baths. Because the NMQSD and HOPS provide access to the global unitary
dynamics of system (working medium) and bath, it is predestined to be
put to use in this field.
...
\begin{equation}
  ...
  η = \frac{Δ\ev{H}_{\mathrm{cyc}}}{Δ\ev{H_{\bath}}_{\mathrm{cyc},h}},
\end{equation}
where \(Δ\ev{H_{\bath}}_{\mathrm{cyc},h}\) is the hot bath energy change
...
immediate appeal of this quantity for the purposes of this work
however is its relation to the full unitary dynamics of system
\emph{and} bath which is accessible to us through HOPS.
...
do not enter a periodic steady state where a constant amount of energy
...
\end{figure} This feature must be somewhat robust, as it is also
visible for \(ω_{c}=0.96\) in \cref{fig:omegas_total}. Such behaviour
is not forbidden by the results of \cref{sec:therm_results}, but
nevertheless interesting for future study as it corresponds to a
situation where we modulate the system indefinitely without having to
expend energy.
...
advantageous to design a stroke based or continuously coupled cycle.
