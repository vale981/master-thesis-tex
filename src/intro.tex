...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
The functional \(ψ(η_{t}^\ast)\) is essentially the state of the
...
discussed here but in \cref{sec:nonlin_flow}.  Crucially, the system
state is now recovered through
\begin{equation}
...
They key~\cite{Suess2014Oct,Hartmann2017Dec,RichardDiss} is cast the
complicated term containing the functional derivative into an
auxiliary state. Expanding the BCF into exponentials
\(α(τ)=∑_{μ}G_{μ=1}^{M}\eu^{-W_{μ}τ}\) and defining
\begin{equation}
  \label{eq:d_op_one}
  D_{μ}(t)\equiv ∫_{0}^{t} G_{μ} \eu^{-W_{μ}(t-s)} \fdv{η^\ast_s},\; D^{\vb{k}}(t)\equiv \prod_{μ=1}^{M}\sqrt{\frac{k_{μ}!}{G_{μ}^{k_{μ}}}}
...
cutoff the method can be made arbitrarily exact. For concrete
truncation schemes see \cite{RichardDiss} and \cref{sec:truncsch}. In
most simulations in this work the so called \emph{simplex truncation
  scheme} was used. This scheme amounts to only including hierarchy
states with \(\abs{\vb{k}} \leq k_{\max}\).
...
into a \emph{nonlinear} system of differential equations. Throughout
this work, the nonlinear method is being used, as it offers much
...
Although the existing HOPS code of the Theoretical Quantum Optics
group\footnote{Available upon reasonable request.} created by Richard
Hartmann was already extensive and well tested, some improvements have
been made due to this work.  Documentation, type hints and unit tests
have been introduced. The parallelization mechanism was overhauled and
now uses the \href{https://www.ray.io}{\texttt{Ray}}
