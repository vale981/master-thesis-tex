...
the stochastic process. We have dropped the bra-ket notation to
conform to the notation of
\cite{Diosi1997,Diosi1998Mar,Strunz2001Habil} and to underline the
re-interpretation as a functional of a stochastic process.
...
\begin{equation}
  ...
  ∂_tψ_t(\tilde{η}^\ast_t) = -\iu H ψ_t(\tilde{η}^\ast_t) +
  L {\tilde{η}}^\ast_tψ_t(\tilde{η}^\ast_t) - \pqty{L^† -\ev{L^\dag}_{t}}∫_0^t\dd{s}α(t-s)\fdv{ψ_t({\tilde{η}}^\ast_t)}{\tilde{η}^\ast_s}.
\end{equation}
